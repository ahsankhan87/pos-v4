...
<div class="max-w-3xl mx-auto p-6">
    <h1 class="text-2xl font-bold mb-4"><?= lang('Roles.editRole') ?>: <?= esc($role['name']) ?></h1>
    <form method="post" action="<?= site_url('roles/update/' . $role['id']) ?>">
        ...
        <div class="mt-6">
            <h2 class="font-semibold mb-2"><?= lang('Roles.permissions') ?></h2>
        ...
        <div class="text-right mt-6">
            <button class="bg-blue-600 text-white px-5 py-2 rounded"><?= lang('Roles.saveChanges') ?></button>
        </div>
    </form>
</div>
<?= $this->endSection() ?>