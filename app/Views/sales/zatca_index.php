...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">ZATCA E-Invoices</h1>
            <p class="mt-1 text-sm text-gray-500">Track reported, cleared, pending submissions and API responses.</p>
        </div>
        <div class="mt-4 sm:mt-0">
            <?php if (can('sales.view')): ?>
                <a href="<?= site_url('sales') ?>" class="btn btn-muted mr-2">
                    <i class="fas fa-arrow-left"></i> Sales List
                </a>
            <?php endif; ?>
            <?php if (can('sales.view')): ?>
    ...
    <?php if (session()->getFlashdata('success')): ?>
        <div class="rounded-md bg-green-50 p-4 mb-6">
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-green-800 whitespace-pre-line">
                        <?= esc(session()->getFlashdata('success')) ?>
                    </p>
                </div>
            </div>
        </div>
        <?php if (session()->getFlashdata('compliance_results')): ?>
            <?php
            $complianceData = json_decode(session()->getFlashdata('compliance_results'), true);
            if ($complianceData && isset($complianceData['validation_response'])):
                $response = $complianceData['validation_response'];
            ?>
                <div class="rounded-md bg-blue-50 p-4 mb-6">
                    <details class="cursor-pointer">
                        <summary class="font-semibold text-blue-900 flex items-center gap-2">
                            <span>📋 Full ZATCA Compliance Response</span>
                        </summary>
                        <div class="mt-3 bg-white border border-blue-200 rounded p-3 font-mono text-xs overflow-x-auto">
                            <div class="mb-2"><strong>Invoice #:</strong> <?= esc($complianceData['invoice_no'] ?? '') ?></div>
                            <div class="mb-2"><strong>Checked At:</strong> <?= esc($complianceData['checked_at'] ?? '') ?></div>
                            <div class="mb-2"><strong>UUID:</strong> <?= esc($complianceData['uuid'] ?? '') ?></div>
                            <div class="mb-2"><strong>Invoice Hash:</strong> <?= esc($complianceData['invoice_hash'] ?? '') ?></div>
                            <div class="mb-2"><strong>ICV:</strong> <?= esc($complianceData['icv'] ?? '') ?></div>
                            <hr class="my-2">
                            <div class="mb-2"><strong>Has Errors:</strong> <?= $response['has_errors'] ? 'Yes' : 'No' ?></div>
                            <div class="mb-2"><strong>Has Warnings:</strong> <?= $response['has_warnings'] ? 'Yes' : 'No' ?></div>
                            <?php if (!empty($response['error_messages'])): ?>
                                <div class="mb-2 text-red-700"><strong>❌ Errors:</strong>
                                    <ul class="ml-4 list-disc">
                                        <?php foreach ($response['error_messages'] as $error): ?>
                                            <li><?= esc($error) ?></li>
                                        <?php endforeach; ?>
                                    </ul>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($response['warning_messages'])): ?>
                                <div class="mb-2 text-yellow-700"><strong>⚠️ Warnings:</strong>
                                    <ul class="ml-4 list-disc">
                                        <?php foreach ($response['warning_messages'] as $warning): ?>
                                            <li><?= esc($warning) ?></li>
                                        <?php endforeach; ?>
                                    </ul>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($response['info_messages'])): ?>
                                <div class="mb-2 text-blue-700"><strong>ℹ️ Info:</strong>
                                    <ul class="ml-4 list-disc">
                                        <?php foreach ($response['info_messages'] as $info): ?>
                                            <li><?= esc($info) ?></li>
                                        <?php endforeach; ?>
                                    </ul>
                                </div>
                            <?php endif; ?>
                            <hr class="my-2">
                            <div class="mb-2"><strong>Status:</strong> <?= esc($response['status'] ?? 'Unknown') ?></div>
                            <div class="mb-2"><strong>Message:</strong> <?= esc($response['message'] ?? '') ?></div>
                        </div>
                    </details>
                </div>
            <?php endif; ?>
    ...
    <?php if (session()->getFlashdata('error')): ?>
        <div class="rounded-md bg-red-50 p-4 mb-6">
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-red-800 whitespace-pre-line">
        ...
        <?php if (session()->getFlashdata('compliance_results')): ?>
            ...
            if ($complianceData && isset($complianceData['validation_response'])):
                $response = $complianceData['validation_response'];
            ?>
                <div class="rounded-md bg-red-50 p-4 mb-6 border-2 border-red-200">
                    <details class="cursor-pointer">
                        <summary class="font-semibold text-red-900 flex items-center gap-2">
                            <span>📋 Full ZATCA Compliance Response</span>
                        </summary>
                        <div class="mt-3 bg-white border border-red-200 rounded p-3 font-mono text-xs overflow-x-auto">
                            <div class="mb-2"><strong>Invoice #:</strong> <?= esc($complianceData['invoice_no'] ?? '') ?></div>
                            <div class="mb-2"><strong>Checked At:</strong> <?= esc($complianceData['checked_at'] ?? '') ?></div>
                            <div class="mb-2"><strong>UUID:</strong> <?= esc($complianceData['uuid'] ?? '') ?></div>
                            <div class="mb-2"><strong>Invoice Hash:</strong> <?= esc($complianceData['invoice_hash'] ?? '') ?></div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        const routes = {
            datatable: <?= json_encode(site_url('sales/zatca-datatable')) ?>,
            ...
            const columns = [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                            <a href="${routes.receipt}/${row.id}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline">
                            ${escapeHtml(data)}
                            </a>
                        `;
                    }
        ...
        function buildActions(row) {
            ...
            if (permissions.receiptsView) {
                menuItems += `
                    <a href="${routes.receipt}/${row.id}" target="_blank" rel="noopener noreferrer" class="actions-link actions-link--info">
                        <i class="fas fa-receipt"></i>
                        <span>${salesI18n.viewReceipt}</span>
                    </a>
                    <a href="${routes.invoicePdf}/${row.id}" target="_blank" rel="noopener noreferrer" class="actions-link actions-link--primary">
            ...
            if (showZatcaColumns) {
                ...
                if ((row.zatca_status || '').toLowerCase() === 'signed' || (row.zatca_status || '').toLowerCase() === 'pending') {
                    menuItems += `
                        <form action="${routes.complianceCheck}/${row.id}" method="POST" class="inline">
                            <?= csrf_field() ?>
                            <button type="submit" class="actions-link actions-link--info">
                                <i class="fas fa-shield-check"></i>
                                <span>${salesI18n.complianceCheck}</span>
                            </button>
                        </form>
                    `;
                }
