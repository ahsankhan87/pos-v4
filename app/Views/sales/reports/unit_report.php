...
<div class="max-w-7xl mx-auto print-root">
    <div class="bg-white shadow rounded-lg mb-6">
        <div class="px-6 py-4 border-b border-gray-100 space-y-3">
            <div class="flex flex-col gap-0.5 sm:flex-row sm:items-end sm:justify-between">
                <h2 class="text-xl font-bold text-gray-900"><?= lang('Reports.unit_wise_sales_report') ?></h2>
            ...
            <form method="get" class="no-print grid grid-cols-1 md:grid-cols-12 gap-2 items-end">
                ...
                <div class="md:col-span-5 flex flex-col sm:flex-row gap-2 md:justify-end">
                    <button type="submit" class="inline-flex h-9 items-center justify-center px-3.5 rounded-md bg-blue-600 text-sm text-white hover:bg-blue-700 shadow-soft"><i class="fas fa-filter mr-2"></i> <?= lang('Reports.apply') ?></button>
                    <?php if (can('reports.unit_sales')): ?>
                        <a href="<?= site_url('sales/unit-report/print?from=' . urlencode($from) . '&to=' . urlencode($to) . ($employee_id ? ('&employee_id=' . urlencode($employee_id)) : '')) ?>" target="_blank" class="inline-flex h-9 items-center justify-center px-3.5 rounded-md bg-gray-700 text-sm text-white hover:bg-gray-800 shadow-soft"><i class="fas fa-print mr-2"></i> <?= lang('Reports.print') ?></a>
                    <?php endif; ?>
                </div>
                <?php $empParam = $employee_id ? ('&employee_id=' . urlencode($employee_id)) : ''; ?>
                <!-- <div class="flex items-end gap-2">
                    <?php if (can('reports.export')): ?>
                        <a href="<?= site_url('sales/unit-report/export_pdf?from=' . urlencode($from) . '&to=' . urlencode($to) . $empParam) ?>" class="inline-flex items-center px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700 shadow-soft"><i class="fas fa-file-pdf mr-2"></i> PDF</a>
                        <a href="<?= site_url('sales/unit-report/export_excel?from=' . urlencode($from) . '&to=' . urlencode($to) . $empParam) ?>" class="inline-flex items-center px-4 py-2 rounded-md bg-yellow-400 text-gray-900 hover:bg-yellow-500 shadow-soft"><i class="fas fa-file-csv mr-2"></i> CSV</a>
                    <?php endif; ?>
                </div> -->
        ...
        <div class="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stats-summary">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <div class="text-xs text-gray-600"><?= lang('Reports.units') ?></div>
    ...
    <div class="bg-white shadow rounded-lg print-container">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-900"><?= lang('Reports.totals_by_unit') ?></h3>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><?= lang('Reports.unit') ?></th>
