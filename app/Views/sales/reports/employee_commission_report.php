    ...
    <?php if (!empty($reportData)): ?>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            foreach ($reportData as $sale) {
                ...
                $grandCommission += ($sale['commission_amount'] ?? 0);
                $saleCount++;
            }
            ?>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    foreach ($reportData as $sale) {
                        ...
                        $employeeSales[$employeeName]['total_sales'] += $sale['total_amount'];
                        $employeeSales[$employeeName]['total_commission'] += $sale['commission_amount'] ?? 0;
                    }
                    ?>
                    <?php foreach ($employeeSales as $employeeName => $data): ?>
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?= esc($employeeName) ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?= session()->get('currency_symbol') . number_format($data['total_sales'], 2) ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900"><?= session()->get('currency_symbol') . number_format($data['total_commission'], 2) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden ">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php /* Totals computed above for the KPI cards */ ?>
                    <?php foreach ($reportData as $sale): ?>
                        <tr>
...
<script>
    function quickRange(days) {
        ...
        window.location = '<?= site_url('sales/employee-commission-report') ?>' + '?' + params.toString();
    }
