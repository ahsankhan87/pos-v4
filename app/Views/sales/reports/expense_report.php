...
<div class="max-w-7xl mx-auto p-6 bg-white shadow rounded-lg print-root">
    <div class="flex justify-between items-center mb-6 no-print">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Expense Report</h2>
            <p class="text-gray-600 mt-2">
                From <?= htmlspecialchars($from) ?> to <?= htmlspecialchars($to) ?>
            </p>
        </div>
        <div class="flex gap-3">
            <a href="<?= site_url('sales/expense-report/print') ?>?from=<?= htmlspecialchars($from) ?>&to=<?= htmlspecialchars($to) ?>" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Print</a>
            <a href="<?= site_url('sales/expense-report/export_excel') ?>?from=<?= htmlspecialchars($from) ?>&to=<?= htmlspecialchars($to) ?>" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Export Excel</a>
            <a href="<?= site_url('sales/expense-report/export_pdf') ?>?from=<?= htmlspecialchars($from) ?>&to=<?= htmlspecialchars($to) ?>" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Export PDF</a>
                ...
                <?php else: ?>
                    <?php foreach ($expenses as $expense): ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border border-gray-300 px-4 py-2"><?= htmlspecialchars($expense['category_name'] ?? 'Uncategorized') ?></td>
                    ...
                    <tr class="bg-gray-100 font-bold">
                        <td colspan="4" class="border border-gray-300 px-4 py-2 text-right">TOTALS:</td>
