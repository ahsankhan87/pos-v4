...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-4 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white px-3 py-3 sm:px-4">
            <div class="flex flex-col gap-0.5">
                ...
                <p class="text-xs text-slate-500 sm:text-sm"><?= esc(lang('Reports.debtors_by_area_description')) ?></p>
            </div>
        </div>
        <div class="px-3 py-3 sm:px-4">
            ...
            <div class="mt-2.5 flex flex-col gap-2 border-t border-slate-100 pt-2.5 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div id="debtorsByAreaDateShortcuts" class="flex flex-wrap items-center gap-2">
    ...
    <div class="table-card">
        <div class="overflow-x-auto">
            <table id="debtorsByAreaTable" class="data-table">
                <thead>
                    <tr>
                        <th><?= esc(lang('Reports.area')) ?></th>
                        <th class="text-right"><?= esc(lang('Reports.customer_count')) ?></th>
                ...
                <tfoot>
                    <tr class="font-semibold bg-gray-100">
                        <td><?= esc(lang('Reports.totals_upper')) ?></td>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const routes = {
            data: <?= json_encode(site_url('reports/debtors-by-area/data')) ?>
        };
        ...
        const dtTexts = {
            ...
            debtorsByAreaBalances: <?= json_encode(lang('Reports.debtors_by_area_balances'), JSON_UNESCAPED_UNICODE) ?>,
            debtorsByAreaExportTitle: <?= json_encode(lang('Reports.debtors_by_area_export_title'), JSON_UNESCAPED_UNICODE) ?>
        };
        ...
        const shortcutButtons = Array.from(document.querySelectorAll('#debtorsByAreaDateShortcuts .date-shortcut'));
        ...
        const table = $('#debtorsByAreaTable').DataTable({
            processing: true,
            serverSide: true,
            deferRender: true,
            ...
            buttons: [{
                    ...
                    title: dtTexts.debtorsByAreaBalances,
                    customize: function(win) {
                        var $body = $(win.document.body);
                        var $table = $body.find('table');
                        var $title = $body.find('h1').first();
                        var customerCountTotal = 0;
                        var openingTotal = 0;
                        var debitsTotal = 0;
                        var creditsTotal = 0;
                        var balanceTotal = 0;
                        ...
                        $table.find('tbody tr').each(function() {
                            ...
                            customerCountTotal += parseInt($cells.eq(1).text()) || 0;
                            openingTotal += extractNumber($cells.eq(2).text());
                            debitsTotal += extractNumber($cells.eq(3).text());
                            creditsTotal += extractNumber($cells.eq(4).text());
                            balanceTotal += extractNumber($cells.eq(5).text());
                        });
                        ...
                        $('<tr></tr>').appendTo($tfoot).html(
                            '<td style="font-weight:bold;">' + dtTexts.totalsUpper + '</td>' +
                            '<td style="text-align:right;font-weight:bold;">' + customerCountTotal + '</td>' +
                            '<td style="text-align:right;font-weight:bold;">' + currency + openingTotal.toFixed(2) + '</td>' +
                            '<td style="text-align:right;font-weight:bold;">' + currency + debitsTotal.toFixed(2) + '</td>' +
                            '<td style="text-align:right;font-weight:bold;">' + currency + creditsTotal.toFixed(2) + '</td>' +
                            '<td style="text-align:right;font-weight:bold;">' + currency + balanceTotal.toFixed(2) + '</td>'
                        );
                ...
                },
                {
                    ...
                    title: dtTexts.debtorsByAreaExportTitle
                },
                {
                    extend: 'excelHtml5',
            ...
            columns: [{
                    data: 'area',
                    render: function(d) {
                        return d ? $('<div>').text(d).html() : '<span class="text-gray-400 text-xs">—</span>';
                    }
                },
                {
                    data: 'customer_count',
            ...
            language: {
                search: "_INPUT_",
                searchPlaceholder: <?= json_encode(lang('Reports.search_area_placeholder'), JSON_UNESCAPED_UNICODE) ?>,
            ...
            footerCallback: function() {
                ...
                api.column(1).data().each(function(val) {
                    customerCountTotal += parseInt(val) || 0;
                });
                api.column(2).data().each(function(val) {
                ...
                $(api.column(1).footer()).html(customerCountTotal);
                $(api.column(2).footer()).html(currency + openingTotal.toFixed(2));
                $(api.column(3).footer()).html(currency + debitsTotal.toFixed(2));
                $(api.column(4).footer()).html(currency + creditsTotal.toFixed(2));
                $(api.column(5).footer()).html('<span class="' + (balanceTotal >= 0 ? 'text-red-600' : 'text-green-700') + ' font-semibold">' + currency + balanceTotal.toFixed(2) + '</span>');
            }
        });
