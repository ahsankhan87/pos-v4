...
<div class="max-w-5xl mx-auto px-4 py-6">
    <h1 class="text-2xl font-bold mb-4"><?= lang('Billing.plansPricing') ?></h1>
    ...
    <div class="mb-4">
        <a href="<?= site_url('billing/manage') ?>" class="mb-4 inline-block px-3 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">
            &larr; <?= lang('Billing.backToSubscription') ?>
        </a>
    </div>
            ...
            <div class="bg-white shadow rounded p-4">
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-2">
                        ...
                        <?php if ($isCurrent): ?>
                            <span class="px-2 py-0.5 text-xs rounded bg-green-100 text-green-800"><?= lang('Billing.current') ?></span>
                        <?php elseif ($isScheduled): ?>
                            <span class="px-2 py-0.5 text-xs rounded bg-yellow-100 text-yellow-800"><?= lang('Billing.scheduled') ?></span>
                        <?php endif; ?>
                    </div>
                    <span class="text-gray-600">
                        <?= esc($plan['currency']) ?> <?= number_format($plan['price_monthly'], 2) ?>/<?= lang('Billing.perMonth') ?>
                    </span>
                </div>
                <div class="mt-2 text-sm text-gray-700">
                    ...
                    <ul class="list-disc ml-5">
                        <?php foreach ($features as $key => $enabled): ?>
                            <li><?= esc(ucwords(str_replace('_', ' ', $key))) ?>: <?= $enabled ? lang('Billing.yes') : lang('Billing.no') ?></li>
                        <?php endforeach; ?>
                    </ul>
                </div>
                <div class="mt-4">
                    <?php if ($isCurrent): ?>
                        <button class="px-3 py-2 bg-gray-200 text-gray-600 rounded cursor-not-allowed" disabled><?= lang('Billing.currentPlan') ?></button>
                    <?php elseif ($isScheduled): ?>
    ...
    <div class="mt-8">
        <h3 class="text-lg font-semibold mb-2"><?= lang('Billing.haveLicenseCode') ?></h3>
        <a class="px-3 py-2 bg-emerald-600 text-white rounded" href="<?= site_url('billing/activate') ?>"><?= lang('Billing.activateLicense') ?></a>
    </div>
</div>
<?= $this->endSection() ?>